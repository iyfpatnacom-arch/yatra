...
export function buildFilter(searchParams) {
  ...
  /* A facilitator counsels individual travellers, not bookings, so a family of
     four counts for a facilitator if any one of them named them. $and rather
     than $or because the free-text search below already owns the top-level
     $or; Mongo intersects the two. */
  const facilitator = searchParams.get("facilitator");
  if (ALL_FACILITATOR_VALUES.includes(facilitator)) {
    filter.$and = [
      {
        $or: [
          { "primary.facilitator": facilitator },
          { "members.facilitator": facilitator },
        ],
      },
    ];
  }
