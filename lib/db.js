...
/**
 * Atomically hands out the next number in a named sequence, starting at 1.
 *
 * Registration IDs are short enough to be read over a phone (BRAJ-Y-123), and
 * three digits is only 900 values — far too few to pick at random without
 * collisions. A counter makes each one unique by construction, so the
 * registration route never has to retry a duplicate key.
 *
 * `findOneAndUpdate` with upsert is a single atomic operation, so two
 * registrations submitted in the same instant get different numbers even
