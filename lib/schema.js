...
/**
 * The billing address printed on the receipt, asked once per booking.
 *
 * English letters only: the receipt's fonts have no Devanagari, and an address
 * that prints as blank space is worse than one the visitor is asked to retype.
 * Short enough to sit in two lines of the receipt.
 */
export const billingAddressSchema = z
  ...
  .pipe(
    z
      ...
      .max(MAX_ADDRESS_LENGTH, "address_too_long")
      .regex(/^[A-Za-z0-9][A-Za-z0-9 ,./#()'&:-]*$/, "address_invalid")
      // "Patna" alone is a city, not somewhere a receipt can be addressed to.
      .refine((value) => value.length >= 8, "address_short")
      /* A six-digit Indian PIN code somewhere in it, standing on its own: a
         mobile number or a seven-digit run does not count as one. */
      .refine((value) => /(?<!\d)[1-9]\d{5}(?!\d)/.test(value), "address_pincode")
  );
